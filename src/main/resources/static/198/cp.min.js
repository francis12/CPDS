...
ui.g = {
	...
	clear: function() {
		...
		$("#lot-sum").text("0");
		$("#play-prize, #play-price").text("0.00");
		$("#play-prize-wrapper").hide();
		$("#debug-lot").text("0");
		$("#debug-amount, #debug-prize").text("0.00");
		ui.g.setPrize()
	},
	summary: function() {
		var h = c.m,
		...
		if (w.p == -1) {
			if (e > 0) {
				$("#play-prize-wrapper").show();
				$("#play-prize-label").text(l.sys.singlePrize);
				$("#play-prize, #debug-prize").text(u.currency(e))
			} else {
				$("#play-prize-wrapper").hide();
				$("#play-prize, #debug-prize").text("-")
			}
		} else {
			$("#play-prize-wrapper").show();
			$("#play-prize-label").text(l.sys.prize);
			$("#play-prize, #debug-prize").text(u.currency(w.p))
		}
		if (parseFloat(w.p) == 0) {
			ui.g.setPrize()
		}
	...
	setPrize: function() {
		...
		if (a == -1) {
			if (b > 0) {
				$("#play-prize-wrapper").show();
				$("#play-prize-label").text(l.sys.singlePrize);
				$("#play-prize, #debug-prize").text(u.currency(b))
			} else {
				$("#play-prize-wrapper").hide();
				$("#play-prize, #debug-prize").text("-")
			}
			...
			$("#play-prize-wrapper").show();
			$("#play-prize-label").text(l.sys.prize);
			$("#play-prize, #debug-prize").text(u.currency(a))
		}
	},
	normal: function(a) {
		if ($("#debug-lot").text() != "0") {
...
ui.o = {
	...
	clear: function() {
		...
		$(".order-total-rebate, .order-total-amount, #debug-total-amount, #debug-total-rebate, #debug-total-prize, #debug-total-payable").text("0.00");
		ui.o.toggleCart(false)
	},
	...
	_orderRow: function(f, e, a) {
		...
		b.push(e.t);
		b.push('</div><div class="order-row-prize col-15 right hidden">');
		b.push(e.p);
		...
		b.push(e.t);
		b.push("</span></div>");
		b.push('<div><span class="label">最高奖金</span><span class="text currency order-row-prize">');
		b.push(e.p);
		b.push("</span></div></div>");
		return b.join("")
	},
	...
	_updateSummary: function(p) {
		...
		if (a.t > s.o.limit && s.o.limit != 0) {
			...
			$("#debug-total-prize").text(u.currency(f));
	...
	_updatePayoutUnit: function() {
		$.each($("#list-cart-row li"), function() {
			...
			e.find(".order-row-prize").text((a > 0) ? u.currency(a) : "-")
		});
		$.each(ui.o.cache, function() {
			var e = this.cd,
